...
static bool try_get_player_by_peer(ENetPeer* peer, strct_active_player** out_player) {
    for(size_t i = 0; i < ACTIVE_PLAYERS_SIZE; i++) {
        strct_active_player* p = ACTIVE_PLAYERS[i];
...
static bool try_get_master_client(strct_active_player** out_player, char* err_buf, size_t err_buf_size) {
    for(size_t i = 0; i < ACTIVE_PLAYERS_SIZE; i++) {
        strct_active_player* p = ACTIVE_PLAYERS[i];
        if(!is_player_exists_and_connected(p)) {
            continue;
...
static bool try_register_player(strct_active_player* player, size_t* out_index, char* error_buffer, size_t error_buf_size) {
    ...
    if(!is_free_index_found) {
        snprintf(error_buffer, error_buf_size, "there is no free space for player.");
        return false;
    }
...
static bool try_remove_player(ENetPeer* peer, char* error_buffer, size_t error_buf_size) {
    for(size_t i = 0; i < ACTIVE_PLAYERS_SIZE; i++) {
        strct_active_player* p = ACTIVE_PLAYERS[i];
        if(!p->is_exists) {
            continue;
    ...
    snprintf(error_buffer, error_buf_size, "player with this ENetPeer pointer '%p' is not found!", peer);
    return false;
}
...
static void handle_game_state_updated_by_master(ifb_strct_game_state game_state) {
    ...
    for(size_t i = 0; i < ACTIVE_PLAYERS_SIZE; i++) {
        ...
        if(!ifb_ptcl_try_send_msg_packet(IFB_MSG_TYPE_GAME_STATE_UPD, IFB_NET_CHANNEL_STREAM, &sbuf, p->peer, SERVER_HOST, true, true, err_buf, ERR_BUFF_SIZE)) {
...
static void handle_player_register(ENetPeer* peer) {
    ...
    if(!ifb_ptcl_try_send_msg_packet(IFB_MSG_TYPE_RES_USER_INFO, IFB_NET_CHANNEL_CRITICAL, &sbuf, peer, SERVER_HOST, true, true, err, ERR_BUFF_SIZE)) {
        fprintf(stderr, "try_register_player() failed: %s\n", err);
        msgpack_sbuffer_destroy(&sbuf);
        return;
    }
    ...
    if(p.is_master == false) {
        ...
        if(!ifb_ptcl_try_send_msg_packet(IFB_MSG_TYPE_SLAVE_CONNECTED, IFB_NET_CHANNEL_CRITICAL, &sbuf, master->peer, SERVER_HOST, true, true, err, ERR_BUFF_SIZE)) {
            fprintf(stderr, "ifb_ptcl_try_send_msg_packet() failed: %s\n", err);
            msgpack_sbuffer_destroy(&sbuf);
            return;
        }
                    ...
                    if(ifb_ptcl_try_parse_message_by_type(&event, &msg_type, &payload, &payload_size, err_buf, ERR_BUFF_SIZE)) {
                        ...
                        switch(msg_type) {
                            case IFB_MSG_TYPE_REQ_USER_INFO: {
                                ...
                                if(!ifb_try_relay_packet(event.packet, master->peer, IFB_NET_CHANNEL_STREAM, true, err_buf, ERR_BUFF_SIZE)) {
