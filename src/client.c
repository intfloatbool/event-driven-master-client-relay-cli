...
static void update_games_state_by_slave_input(ifb_strct_player_input player_input) {
...
static void attempt_init_current_game_state_by_master() {
    ...
    if(!LAST_PLAYER_INPUTS) {
        ...
        if(LAST_PLAYER_INPUTS == NULL) {
            fprintf(stderr, "attempt_init_current_game_state_by_master(), FATAL: malloc() failed!\n");
            pthread_mutex_unlock(&MTX_LAST_PLAYER_INPUTS);
            return;
        }
...
static void handle_on_slave_connected(ifb_strct_user_info user_info) {
    ...
    if(CURRENT_GAME_STATE == NULL) {
        fprintf(stderr, "handle_on_slave_connected(): CURRENT_GAME_STATE is NULL\n");
        pthread_mutex_unlock(&MTX_CURRENT_GAME_STATE);
        return;
    }
...
static void handle_on_slave_game_state_updated(ifb_strct_game_state game_state) {
    puts("\n*** *** game_state updated *** ***\n");
    printf("\t tick: %d\n", game_state.tick);
    printf("\t pos_1: ( exists: %d, x: %d, y: %d )\n", game_state.pos_1.is_exists, game_state.pos_1.x, game_state.pos_1.y);
    printf("\t pos_2: ( exists: %d, x: %d, y: %d )\n", game_state.pos_2.is_exists, game_state.pos_2.x, game_state.pos_2.y);
    printf("\t pos_3: ( exists: %d, x: %d, y: %d )\n", game_state.pos_3.is_exists, game_state.pos_3.x, game_state.pos_3.y);
    printf("\t pos_4: ( exists: %d, x: %d, y: %d )\n", game_state.pos_4.is_exists, game_state.pos_4.x, game_state.pos_4.y);
    puts("\n*** *** *** *** *** *** ** *** ***\n");
}
...
static void handle_cmd_slave_tick(strct_user_input* user_input) {
    ...
    if(!ifb_ptcl_try_send_msg_packet(IFB_MSG_TYPE_PLAYER_INPUT, IFB_NET_CHANNEL_STREAM, &sbuf, CLIENT_PEER, CLIENT_HOST, true, true, err_buf, ERR_BUFF_SIZE)) {
        fprintf(stderr, "handle_cmd_slave_tick() err:\n\t%s\n", err_buf);
        msgpack_sbuffer_destroy(&sbuf);
        return;
    }
...
static void handle_cmd_connect_to_host(strct_msg_connect* msg) {
    ...
    ENetHost* client = enet_host_create(
        NULL, // client identified with NULL
    ...
    ENetPeer* peer = enet_host_connect(client, &address, IFB_CNST_NET_CHANNELS_COUNT, 0);
...
static void handle_net_events_loop() {
    ...
    switch(event.type) {
        case ENET_EVENT_TYPE_RECEIVE: {
            ...
            char err_buf[ERR_BUFF_SIZE];
            if(ifb_ptcl_try_parse_message_by_type(&event, &msg_type, &payload, &payload_size, err_buf, ERR_BUFF_SIZE)) {
                ...
                switch(msg_type) {
                    case IFB_MSG_TYPE_RES_USER_INFO: {
                        ...
                        if(ifb_fun_try_unpack_user_info(payload, payload_size, &usr_info, err_buf, ERR_BUFF_SIZE)) {
                            pthread_mutex_lock(&MTX_MY_USER_INFO);
                        ...
                        ifb_strct_game_state game_state = {0};
                        if(ifb_try_unpack_game_state(payload, payload_size, &game_state, err_buf, ERR_BUFF_SIZE, false)) {
                        ...
                        if(ifb_fun_try_unpack_user_info(payload, payload_size, &ui, err_buf, ERR_BUFF_SIZE)) {
                        ...
                        ifb_strct_player_input player_input = {0};
                        if(ifb_try_unpack_player_input(payload, payload_size, &player_input, err_buf, ERR_BUFF_SIZE)) {
...
static void* net_bg_thread(void* arg) {
    ...
    while(atomic_load(&IS_APP_RUNNING)) {
        ...
        if(ifb_safe_queue_try_pop(LOCAL_COMMANDS_SF_QUEUE, &queue_data)) {
            ...
            strct_client_command_msg* cmd_msg = (strct_client_command_msg*) queue_data;
            if(cmd_msg == NULL) {
            ...
            switch(cmd_msg->type) {
                ...
                case DISCONNECT_N_FREE: {
                    ...
                    if(!ifb_ptcl_try_send_msg_packet(IFB_MSG_TYPE_REQ_USER_INFO, IFB_NET_CHANNEL_CRITICAL, NULL, CLIENT_PEER, CLIENT_HOST, true, true, err, ERR_BUFF_SIZE)) {
                    ...
                    if(!ifb_ptcl_try_send_msg_packet(IFB_MSG_TYPE_GAME_STATE_UPD, IFB_NET_CHANNEL_STREAM, &sbuf, CLIENT_PEER, CLIENT_HOST, true, true, err, ERR_BUFF_SIZE)) {
                        printf("%s send_msg_packet_to_server error: %s\n", NET_SUFFIX, err);
                    }
                    msgpack_sbuffer_destroy(&sbuf);
                    break;
...
static bool try_execute_tick_cmd(uint32_t ticks, strct_user_input user_input, char* err_buf, size_t err_buf_size) {
...
int main(int argc, char** argv) {
    ...
    while(true) {
        ...
        if(fgets(input_buf, sizeof(input_buf), stdin) != NULL) {
            // находим символ новой строки, который добавляется fgets() и заменяем его символом конца строки (null-terminate).
