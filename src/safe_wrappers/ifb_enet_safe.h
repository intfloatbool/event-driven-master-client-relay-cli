...
static inline bool try_enet_peer_send(ENetPeer *peer, enet_uint32 channel_id,
                                      ENetPacket *packet, char *err_buf,
                                      size_t err_buf_size) {
  ...
  if (peer->state != ENET_PEER_STATE_CONNECTED) {
    snprintf(err_buf, err_buf_size,
             "try_enet_peer_send() peer state is not connected. Actual: %d",
             peer->state);
    return false;
  }
  ...
  if (channel_id >= peer->channelCount) {
    snprintf(
        ...
        "try_enet_peer_send() channel_id is larger than peer->channelCount");
    return false;
  }

  if (packet->dataLength > peer->host->maximumPacketSize) {
    snprintf(err_buf, err_buf_size,
             "try_enet_peer_send() cpacket->dataLength is larger than "
             "host->maximumPacketSize (%lu / %lu)",
