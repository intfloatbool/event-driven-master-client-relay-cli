...
bool ifb_ptcl_try_send_msg_packet(ifb_en_message_type msg_type, ifb_en_channel_id channel_id, msgpack_sbuffer* sbuf, ENetPeer* peer, ENetHost* host, bool is_reliable, bool is_forced, char* error_buffer, size_t error_buf_size) {
    ...
    switch(msg_type) {
        case IFB_MSG_TYPE_REQ_USER_INFO: {
            ...
            snprintf(error_buffer, error_buf_size, "unknown ifb_en_message_type msg_type!");
            return false;
        }
    }
    ...
    ENetPacket* pkt = enet_packet_create(
        data,
        data_size,
        is_reliable ? ENET_PACKET_FLAG_RELIABLE : ENET_PACKET_FLAG_UNRELIABLE_FRAGMENT
    );
...
bool ifb_try_relay_packet(ENetPacket* pkt_src, ENetPeer* peer_to, ifb_en_channel_id channel_id, bool is_reliable, char* err_buf, size_t err_buf_size) {
    ENetPacket* pkt = enet_packet_create(
        pkt_src->data,
        pkt_src->dataLength,
        is_reliable ? ENET_PACKET_FLAG_RELIABLE : ENET_PACKET_FLAG_UNRELIABLE_FRAGMENT
...
bool ifb_ptcl_try_parse_message_by_type(ENetEvent* event, ifb_en_message_type* out_msg_type, uint8_t** out_only_msg_payload, size_t* out_only_msg_payload_size, char* err_buf, size_t err_buf_size) {
...
bool ifb_try_pack_user_info(msgpack_sbuffer* sbuf, ifb_strct_user_info* user, char* err_buf, size_t err_buf_size) {
    ...
    if(msgpack_pack_str_body(&pk, is_master_key, strlen(is_master_key)) != 0) {
        snprintf(err_buf, err_buf_size, "msgpack_pack_str_body 'is_master' KEY failed.");
        return false;
    }
    //value
    int pack_bool_res = -1;
    ...
    if(pack_bool_res != 0) {
        snprintf(err_buf, err_buf_size, "msgpack_pack true/false 'is_master' VALUE failed.");
        return false;
    }
...
bool ifb_fun_try_unpack_user_info(const uint8_t* data, size_t size, ifb_strct_user_info* out_user_info, char* err_buf, size_t err_buf_size) {
    ...
    switch( msgpack_unpack_next(&result, (const char*) data, size, &offset) ) {
        ...
        case MSGPACK_UNPACK_CONTINUE: {
            snprintf(err_buf, err_buf_size, "msgpack_unpack_next() err: UNPACK_CONTINUE");
            msgpack_unpacked_destroy(&result);
            return false;
        }
        default: {
    ...
    for(uint32_t i = 0; i < root.via.map.size; i++) {
        ...
        } else if(key_str_len == is_master_key_len && memcmp(key_str, is_master_key, is_master_key_len) == 0) {
    ...
    if(!is_f_id_parsed || !is_f_is_master_parsed) {
        snprintf(err_buf, err_buf_size, "msgpack_unpack_next() err: some value not found.");
        msgpack_unpacked_destroy(&result);
        return false;
    }
...
bool ifb_try_pack_str_full(msgpack_packer* pk, const char* str_value, char* err_buf, size_t err_buf_size) {
...
bool ifb_try_pack_player_pos(msgpack_packer* pk, const ifb_strct_player_pos* pos, char* err_buf, size_t err_buf_size) {   
    size_t keys_count = 3;
    if(msgpack_pack_map(pk, keys_count) != 0) {
        snprintf(err_buf, err_buf_size, "msgpack_pack_map() failed.");
        return false;
    ...
    if(pack_res != 0) {
        snprintf(err_buf, err_buf_size, "msgpack_pack_true/false 'is_exists' VALUE failed.");
        return false;
    }
...
bool ifb_try_pack_player_pos_count(msgpack_packer* pk, ifb_strct_player_pos** pos_ptr_arr, size_t pos_ptr_arr_size, char* err_buf, size_t err_buf_size) {
...
bool ifb_try_pack_game_state(msgpack_sbuffer* sbuf, ifb_strct_game_state* game_state, char* err_buf, size_t err_buf_size) {
    ...
    if(!ifb_try_pack_player_pos_count(&pk, positions, pos_count, err_buf, err_buf_size)) {
        return false;
    }
...
bool ifb_try_unpack_player_pos(msgpack_object obj, ifb_strct_player_pos* out, char* err_buf, size_t err_buf_size) {
    ...
    if(!is_f_is_exists_parsed) {
        snprintf(err_buf, err_buf_size, "field: '%s' is not parsed.", is_exists_key);
        return false;
    }
...
bool ifb_try_unpack_game_state(const uint8_t* data, size_t data_size, ifb_strct_game_state* out_game_state, char* err_buf, size_t err_buf_size, bool is_log_enabled) {
    ...
    size_t offset = 0;
    switch(msgpack_unpack_next(&result, (const char*) data, data_size, &offset)) {
        case MSGPACK_UNPACK_NOMEM_ERROR: {
            snprintf(err_buf, err_buf_size, "msgpack_unpack_next() err: NOMEM_ERROR");
            return false;
        } case MSGPACK_UNPACK_PARSE_ERROR: {
    ...
    msgpack_object root = result.data;
    if(root.type != MSGPACK_OBJECT_MAP) {
        snprintf(err_buf, err_buf_size, "invalid root type: %u", (uint8_t) root.type);
        msgpack_unpacked_destroy(&result);
        return false;
    }
    ...
    for(uint32_t i = 0; i < root.via.map.size; i++) {
        ...
        const char* key_str = k.via.str.ptr;
        size_t key_str_len = k.via.str.size;
        if(key_str_len == tick_key_len && memcmp(key_str, tick_key, tick_key_len) == 0) {
            if(v.type == MSGPACK_OBJECT_POSITIVE_INTEGER) {
            ...
            if(memcmp(key_str, pos_1_key, pos_key_len) == 0 ) {
                if(!ifb_try_unpack_player_pos(v, &unpack_res.pos_1, err_buf, err_buf_size)) {
                    msgpack_unpacked_destroy(&result);
                    return false;
                }
            ...
            if(memcmp(key_str, pos_2_key, pos_key_len) == 0 ) {
                if(!ifb_try_unpack_player_pos(v, &unpack_res.pos_2, err_buf, err_buf_size)) {
                    msgpack_unpacked_destroy(&result);
                    return false;
                }
            ...
            if(memcmp(key_str, pos_3_key, pos_key_len) == 0 ) {
                if(!ifb_try_unpack_player_pos(v, &unpack_res.pos_3, err_buf, err_buf_size)) {
                    msgpack_unpacked_destroy(&result);
                    return false;
                }
            ...
            if(memcmp(key_str, pos_4_key, pos_key_len) == 0 ) {
                if(!ifb_try_unpack_player_pos(v, &unpack_res.pos_4, err_buf, err_buf_size)) {
                    msgpack_unpacked_destroy(&result);
                    return false;
                }
...
bool ifb_try_pack_player_input(msgpack_sbuffer* sbuf, ifb_strct_player_input player_input, char* err_buf, size_t err_buf_size) {
    msgpack_packer pk;
    msgpack_packer_init(&pk, sbuf, msgpack_sbuffer_write);

    size_t keys_count = 3;
    if(msgpack_pack_map(&pk, keys_count) != 0) {
        snprintf(err_buf, err_buf_size, "ifb_try_pack_player_input()->msgpack_pack_map() failed.");
        return false;
    }
    ...
    if(msgpack_pack_uint32(&pk, player_input.id) != 0) {
        snprintf(err_buf, err_buf_size, "ifb_try_pack_player_input() pack player_input.id failed.");
        return false;
    }
    ...
    if(msgpack_pack_int32(&pk, player_input.x) != 0) {
        snprintf(err_buf, err_buf_size, "ifb_try_pack_player_input() pack player_input.x failed.");
        return false;
    }
    ...
    if(msgpack_pack_int32(&pk, player_input.y) != 0) {
        snprintf(err_buf, err_buf_size, "ifb_try_pack_player_input() pack player_input.y failed.");
        return false;
    }
...
bool ifb_try_unpack_player_input(const uint8_t* data, size_t data_size, ifb_strct_player_input* out, char* err_buf, size_t err_buf_size) {
    ...
    switch(msgpack_unpack_next(&result, (const char*) data, data_size, &offset)) {
        case MSGPACK_UNPACK_NOMEM_ERROR: {
            snprintf(err_buf, err_buf_size, "ifb_try_unpack_player_input() unpack error: NOMEM_ERROR");
            return false;
        } case MSGPACK_UNPACK_PARSE_ERROR: {
            snprintf(err_buf, err_buf_size, "ifb_try_unpack_player_input() unpack error: PARSE_ERROR");
            msgpack_unpacked_destroy(&result);
            return false;
        } case MSGPACK_UNPACK_CONTINUE: {
            snprintf(err_buf, err_buf_size, "ifb_try_unpack_player_input() unpack error: UNPACK_CONTINUE");
            msgpack_unpacked_destroy(&result);
            return false;
        } default: {
            break;
    ...
    msgpack_object root = result.data;
    if(root.type != MSGPACK_OBJECT_MAP) {
        snprintf(err_buf, err_buf_size, "ifb_try_unpack_player_input() invalid root type.");
        msgpack_unpacked_destroy(&result);
        return false;
    }
    ...
    for(uint32_t i = 0; i < root.via.map.size; i++) {
        ...
        if(key_str_len == strlen(id_key) && memcmp(key_str, id_key, key_str_len) == 0) {
            if(v.type == MSGPACK_OBJECT_POSITIVE_INTEGER) {
    ...
    if(!is_f_id_parsed) {
        snprintf(err_buf, err_buf_size, "ifb_try_unpack_player_input() 'id' is not parsed.");
        msgpack_unpacked_destroy(&result);
        return false;
    }
    if(!is_f_x_parsed) {
        snprintf(err_buf, err_buf_size, "ifb_try_unpack_player_input() 'x' is not parsed.");
        msgpack_unpacked_destroy(&result);
        return false;
    }
    if(!is_f_y_parsed) {
        snprintf(err_buf, err_buf_size, "ifb_try_unpack_player_input() 'y' is not parsed.");
        msgpack_unpacked_destroy(&result);
        return false;
    }
    *out = unpack_res;
